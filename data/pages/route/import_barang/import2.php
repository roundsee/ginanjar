...
if (isset($_POST['submit'])) {
    ...
    if ($upload) {
        ...
        // Array untuk menyimpan kode pasien yang sudah ditampilkan alert-nya
        $checked_pasien = [];
        ...
        foreach ($all_data as $index => $data) {
            ...
            // Cek apakah kode_barang sudah ada di tabel barang
            ...
            if ($check_barang_data['count'] > 0) {
                // Jika kode_barang sudah ada, lakukan update
                $sql_update_barang = "
                    UPDATE barang 
                    SET 
                        nama='$nama',
                ...
                ";
                mysqli_query($koneksi, $sql_update_barang) or die(mysqli_error($koneksi));
            } else if (!empty($kd_barang)) {
                // Jika kode_barang belum ada, lakukan insert
                $sql_insert_barang = "
                    INSERT INTO barang 
                ...
                ";
                mysqli_query($koneksi, $sql_insert_barang) or die(mysqli_error($koneksi));
            }

            // Insert ke tabel transaksi dan transaksi_detail tetap dilakukan seperti sebelumnya
            // ...

            // Insert ke tabel nasabah tetap dilakukan seperti sebelumnya
            // ...
        }
