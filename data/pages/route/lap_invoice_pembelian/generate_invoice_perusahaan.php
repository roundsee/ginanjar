...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    function generateInvoiceNumber($koneksi)
    {
        ...
        $bulan_romawi = convertToRoman(date('n')); // Menggunakan fungsi untuk mengonversi bulan ke format Romawi
        ...
        $nomor_urut = str_pad($last_sequence + 1, 4, '0', STR_PAD_LEFT); // Mengatur panjang menjadi 4 digit
    ...
    if (isset($_POST['selected_patient']) && !empty($_POST['selected_patient'])) {
        ...
        // Variabel untuk kode perusahaan, nilai, tgl_awal, dan tgl_akhir
        $nilai = mysqli_real_escape_string($koneksi, $_GET['nilai']); // Escape untuk mencegah SQL Injection
        ...
        foreach ($selected_patients as $selected) {
            ...
            $query_get_kode_perusahaan = "SELECT DISTINCT kode_asuransi, kode_pasien, no_card FROM transaksi WHERE kode_pasien = '$kode_pasien' AND tanggal = '$tanggal' AND kode_asuransi = '$nilai'";
            ...
            if ($result_kode_perusahaan && mysqli_num_rows($result_kode_perusahaan) > 0) {
                ...
                $query_insert_invoice = "INSERT INTO invoice (no_invoice, tanggal_invoice, kode_perusahaan, kode_pasien) VALUES ('$no_invoice', '$tanggal_invoice', '$nilai', '$kode_pasien')";
                if (mysqli_query($koneksi, $query_insert_invoice)) {
                    ...
                    if ($result_detail) {
                        while ($row = mysqli_fetch_assoc($result_detail)) {
                            ...
                            $query_insert_detail = "INSERT INTO invoice_detail (no_invoice, item, nilai, tanggal, no_card, tindakan) VALUES ('$no_invoice', '$group', '$nilai_item', '$tanggal', '$no_card', '$tindakan')";
                        ...
                        $query_update = "UPDATE transaksi 
                                        ...
                                        WHERE kode_pasien = '$kode_pasien' AND kode_asuransi = '$nilai' AND tanggal = '$tanggal'";
