                                                        ...
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tbody>
                                                                ...
                                                                while ($s1 = mysqli_fetch_array($sql1)) {
                                                                    // Perhitungan tanpa konversi float
                                                                    $total_nilai = $s1['total_nilai'];
                                                                    $total_disc = $s1['total_disc'];
                                                                    $ppn = $s1['ppn'];
                                                                    $tarif_ppn = $s1['tarif_ppn'];
                                                                    $ongkir = $s1['ongkir'];
                                                                    $nilai_retur = $s1['nilai_retur'];

                                                                    // Hitung subtotal (total nilai barang dikurangi diskon)
                                                                    $subtotal = $total_nilai - $total_disc;

                                                                    // Hitung total PPN
                                                                    $ppn_total = $ppn * ($subtotal * ($tarif_ppn / 100));

                                                                    // Hitung harga total invoice
                                                                    $hargatotal = $subtotal + $ppn_total + $ongkir - $nilai_retur;

                                                                    // Ambil total pembayaran untuk invoice ini
                                                                    $sql2 = mysqli_query($koneksi, "SELECT SUM(jumlah_payment) AS jumlah_payment FROM payment WHERE no_invoice = '$s1[no_invoice]'");
                                                                    $s2 = mysqli_fetch_assoc($sql2);
                                                                    $jumlah_payment = isset($s2['jumlah_payment']) ? $s2['jumlah_payment'] : 0;

                                                                    // Hitung sisa tagihan
                                                                    $sisaTagihan = $hargatotal - $jumlah_payment;

                                                                    // Hitung tanggal jatuh tempo
                                                                    $tanggal_invoice = $s1['tanggal_invoice'];
                                                                    $term = $s1['term_of_payment'];
                                                                    $jatuh_tempo = date('Y-m-d', strtotime($tanggal_invoice . " +$term days"));
                                                                ?>
                                                                    <tr align="left">
                                                                        <td><?php echo $no; ?></td>
                                                                        <td><a href="main.php?route=<?php echo $view; ?>&act&id=<?php echo $s1['no_invoice']; ?>&asal=<?php echo $rute; ?>" title="Detail"><?php echo $s1['no_invoice']; ?></a></td>
