...
if ($act == 'rata_rata') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['selected_patient'])) {
            ...
            $selectedData = $_POST['selected_patient'];
            // Jika tidak ada checkbox yang dipilih, tampilkan alert dan kembali ke form
            $selectedCount = 0;
            foreach ($selectedData as $kd_brg => $data) {
            ...
            foreach ($selectedData as $kd_brg => $data) {
                // Hanya proses jika checkbox dipilih
                if (isset($data['selected']) && $data['selected'] == '1') {
                    $tanggal_beli = date('Y-m-d'); // Tanggal pembelian hari ini


                    // echo "Kode Barang: " . htmlspecialchars($kd_brg) . "<br>";
                    // echo "No: " . htmlspecialchars($data['no']) . "<br>";
                    // echo "Nama Barang: " . htmlspecialchars($data['nama_barang']) . "<br>";
                    // echo "Buffer: " . htmlspecialchars($data['buffer']) . "<br>";
                    // echo "Stok Akhir: " . htmlspecialchars($data['stok_akhir']) . "<br>";
                    // echo "Perhitungan Stok Akhir: " . htmlspecialchars($data['perhitungan_stok_akhir']) . "<br>";
                    // echo "Rata Minggu I: " . htmlspecialchars($data['rata_minggu_I']) . "<br>";
                    // echo "Rata Minggu II: " . htmlspecialchars($data['rata_minggu_II']) . "<br>";
                    // echo "Rata Minggu III: " . htmlspecialchars($data['rata_minggu_III']) . "<br>";
                    // echo "Rata Minggu IV: " . htmlspecialchars($data['rata_minggu_IV']) . "<br>";
                    // echo "Max Rata per Minggu: " . htmlspecialchars($data['max_rata_per_minggu']) . "<br>";
                    // echo "Waktu Kirim Supplier: " . htmlspecialchars($data['kd_supp']) . "<br>";
                    // echo "Waktu Kirim Supplier: " . htmlspecialchars($data['waktu_kirim_supplier']) . "<br>";
                    // echo "Qty Order: " . htmlspecialchars($data['qty_order']) . "<br>";
                    // echo "Estimasi Tidak Aman: " . htmlspecialchars($data['estimasi_tidak_aman']) . "<br>";

                    $kd_supp = $data['kd_supp'];
                    $waktu_kirim_supplier = $data['waktu_kirim_supplier'];
                    $current_order_rata = $data['qty_order'];
                    ...
                    if (!isset($supplier_info[$kd_supp][$tanggal_beli])) {
                        ...
                        $supplier_info[$kd_supp][$tanggal_beli] = [
                            'kd_beli' => $kd_beli,
                            'kd_po' => $kd_po
                        ];
                        ...
                        VALUES ('$kd_cus','$kd_beli', '$kd_supp', '$waktu_kirim_supplier', '$tanggal_beli', '$kd_po' , '$employee')";
                        ...
                        // Gunakan kode beli dan kode PO yang sudah ada untuk kd_supp dan tgl_beli ini
                        $kd_beli = $supplier_info[$kd_supp][$tanggal_beli]['kd_beli'];
                        $kd_po = $supplier_info[$kd_supp][$tanggal_beli]['kd_po'];
                    }
                    ...
                    VALUES ('$kd_beli', '$kd_brg', '$current_order_rata','$price', 'Pcs', '$current_order_rata', '$kd_po')";
                    mysqli_query($koneksi, $query_insert_pembelian_detail);
                }
            }
        } else {
            echo "<script>Tidak ada barang yang dipilih.";
        }
    }
    // die();
} elseif ($act == 'tertinggi') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Cek jika ada checkbox yang dipilih
        if (isset($_POST['selected_patient'])) {
            // Ambil data dari checkbox yang dipilih
            ...
            foreach ($selectedData as $kd_brg => $data) {
                ...
                if (isset($data['selected']) && $data['selected'] == '1') {
                    ...
                    if (!isset($supplier_info[$kd_supp][$tanggal_beli])) {
                        ...
                        // Masukkan data ke tabel pembelian
                        $query_insert_pembelian = "
                        INSERT INTO pembelian (kd_beli, kd_supp, durasi_kirim, tgl_beli, kd_po, user_input)
                        VALUES ('$kd_beli', '$kd_supp', '$waktu_kirim_supplier', '$tanggal_beli', '$kd_po' , '$employee')";
                        // echo $query_insert_pembelian . "<br>";
                        $result = mysqli_query($koneksi, $query_insert_pembelian);
