<script>
    $(document).ready(function() {
        ...
        // pilih kota
        $(document).on("click", ".modal-pilih-asuransi", function() {

            var id = $(this).attr('id');
            var kode = $(this).attr('kode');
            var nama = $(this).attr('nama');

            $("#tampil_asuransi_id").val(id);
            $("#tampil_asuransi_kode").val(kode);
            $("#tampil_asuransi_kode2").val(kode);
            $("#tampil_asuransi_nama").val(nama);

            $("#tampil_outlet_id").val("");
            $("#tampil_area_id").val("");

            $("#status_asuransi").html("<small style='color:green;font-style:italic'>ditemukan</small>");
        });
        ...
        $(document).on("click", ".modal-pilih-area", function() {
            ...
            $("#status_area").html("<small style='color:green;font-style:italic'>Area ditemukan</small>");
        });

        // pilih kasir
        $(document).on("click", ".modal-pilih-kasir", function() {

            var id = $(this).attr('id');
            var kode = $(this).attr('kode');
            var nama = $(this).attr('nama');

            $("#tampil_kasir_id").val(id);
            $("#tampil_kasir_kode").val(kode);
            $("#tampil_kasir_kode2").val(kode);
            $("#tampil_kasir_nama").val(nama);

            $("#status_kasir").html("<small style='color:green;font-style:italic'>Kasir ditemukan</small>");
        });
