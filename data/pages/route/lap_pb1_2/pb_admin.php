...
<?php } elseif ($filter == 'area') { ?>
	...
	<table id="example" class="table table-bordered table-striped">
		...
		<tbody>
			...
			join pelanggan on pelanggan.kd_cus=penjualan.kd_cus 
			join kotabaru on kotabaru.kode=pelanggan.kd_kota 
			WHERE (tanggal between '$tgl_awal' and '$tgl_akhir' +interval 1 day) AND kotabaru.kd_area='$nilai' GROUP BY pelanggan.kd_cus
			HAVING sum(penjualan.subjumlah) <> 0
			ORDER by kotabaru.kode,pelanggan.kd_cus";
...
<?php } elseif ($filter == 'semua' and $nilai == 'semua') { ?>
	...
	<table id="example" class="table table-bordered table-striped">
		...
		<tbody>
			...
			WHERE (tanggal between '$tgl_awal' and '$tgl_akhir' +interval 1 day)  GROUP BY pelanggan.kd_cus 
			HAVING sum(penjualan.subjumlah) <> 0
			ORDER by area.kode,kotabaru.kode,pelanggan.kd_cus";
