...
?>
<script>
  localStorage.setItem('search_menu_value', "");
</script>
...
// $query=mysqli_query($koneksi,"SELECT * FROM penjualan WHERE kd_cus='$kd_cus' AND faktur = '$keyword'  ");
$query = mysqli_query($koneksi, "SELECT * FROM daily_penjualan WHERE kd_cus='$kd_cus' AND faktur = '$keyword'  ");

$qq = mysqli_num_rows($query);

// echo '<br> ketemu='.$qq;

// if(empty($_POST["kd_aplikasi"]) AND $qq!=1)
if ($qq != 0) {
?>
  <div id="show_search">
    <br>
    <!-- <table class="table-responsive example1"  style="height:286px"> -->
    <div class="wrapper bg-white box-poly-up" style="padding: 5px 25px 5px 20px;min-height: 290px!important ;">
      <div class="row table-container">
        <div style="height:auto;background-color: beige;width: 630px;">
          <table class="table table-bordered table-striped " style="height:auto;">
            <thead>
              ...
              <th width="100px" style="text-align: right;">Total Diskon</th>
            ...
            <tbody>
              ...
              $data =  mysqli_query($koneksi, "SELECT *,
                    ...
                    b.nama as b_nama 
                    FROM daily_penjualan p
                    JOIN daily_jualdetil jd ON jd.faktur=p.faktur 
                    JOIN barang b ON b.kd_brg=jd.kd_brg
                    WHERE p.faktur = '$keyword' ");
      ...
      $tot_sel = round($tot_penjualan + $pb1);
      ...
      <div class="col-lg-7 box-poly pull-right" style="padding:14 10 5 14">
        <table class="table table-striped" style="padding-bottom: 1px;padding-bottom: 1px;">
          ...
          <tr>
            <td>Total Diskon</td>
            <td align="right"><?php echo format_rupiah($tot_diskon); ?></td>
          </tr>
  ...
  // echo '<br> cek ='. $cek;
  if ($cek >= 1) {
    echo '<script>show_print()</script>';
  } else {
    echo '<script>hide_print()</script>';
  }
} else {
  // echo '<option value="">Grup not available</option>';
  echo '<h1><center>Data tersebut tdk ada </center></h1>';
  echo '<script>hide_print()</script>';
}
  ?>