...
<!-- <div class="filter-container p-0 row" style="height:186px!important;"> -->
  ...
  $keyword_refund=$keyword.'R';
  // echo '<br> keyword='.$keyword;
  // echo '<br> keyword_refund='.$keyword_refund;

  $query=mysqli_query($koneksi,"SELECT * FROM penjualan WHERE tanggal>='$tgl_hari_ini' AND tanggal<='$tgl_hari_ini' +interval 1 day  AND kd_cus='$kd_cus' AND faktur_refund = '$keyword' OR faktur_void = '$keyword' ");
  ...
  { 
    ...
    <div id="show_search">
      ...
      <!-- <table class="table-responsive example1"  style="height:286px"> -->
              ...
              <table class="table table-bordered table-striped " style="height:auto;">
                ...
                <tbody>
                  ...
                  $data =  mysqli_query($koneksi,"SELECT *,
                    ...
                    WHERE p.tanggal>='$tgl_hari_ini' AND p.tanggal<='$tgl_hari_ini' +interval 1 day  AND p.faktur = '$keyword' ");
        ...
        // echo '<br> cek ='. $cek;
        if($cek>=1){
          echo '<script>show_refund()</script>';
        }else{
          echo '<script>hide_refund()</script>';
        }


      }else{ 
        // echo '<option value="">Grup not available</option>';
        echo '<h1><center>Data tersebut sudah pernah di Refund/Void </center></h1>';
        echo '<script>hide_refund()</script>';
